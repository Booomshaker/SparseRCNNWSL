...
class DynamicHeadWSL(nn.Module):
    ...
    def __init__(self, cfg, roi_input_shape):
        ...
        activation = cfg.MODEL.SparseRCNN.ACTIVATION
        num_heads = cfg.MODEL.SparseRCNN.NUM_HEADS
        rcnn_head = RCNNHeadWSL(cfg, d_model, num_classes, dim_feedforward, nhead, dropout, activation)        
        self.head_series = _get_clones(rcnn_head, num_heads)
        self.use_intermediate = cfg.MODEL.SparseRCNN.DEEP_SUPERVISION
    ...
    def forward(self, features, init_bboxes, init_features):
        ...
        for rcnn_head in self.head_series:
            class_logits, pred_bboxes, proposal_features, proposal_boxes, pred_deltas = rcnn_head(features, bboxes, proposal_features, self.box_pooler)

            if self.use_intermediate:
                inter_class_logits.append(class_logits)
                inter_pred_bboxes.append(pred_bboxes)
                inter_proposal_boxes.append(proposal_boxes)
                inter_pred_deltas.append(pred_deltas)
                bboxes = pred_bboxes.detach()
                roi_features = self.box_pooler(features, [Boxes(bbox) for bbox in bboxes])
                inter_roi_features.append(roi_features)
            bboxes = pred_bboxes.detach()

        if self.use_intermediate:
            return torch.stack(inter_class_logits), torch.stack(inter_pred_bboxes), \
                torch.stack(inter_roi_features), torch.stack(inter_proposal_boxes), \
                torch.stack(inter_pred_deltas)

        roi_features = self.box_pooler(features, [Boxes(bbox) for bbox in bboxes])

        return class_logits[None], pred_bboxes[None], roi_features[None]


class RCNNHeadWSL(nn.Module):
    ...
    def __init__(self, cfg, d_model, num_classes, dim_feedforward=2048, nhead=8, dropout=0.1, activation="relu",
        ...
        nn.init.constant_(self.bboxes_delta.bias, 0)
    ...
    def forward(self, features, bboxes, pro_features, pooler):
        ...
        return class_logits.view(N, nr_boxes, -1), pred_bboxes.view(N, nr_boxes, -1), obj_features, bboxes, bboxes_deltas
...
class BoxHead(nn.Module):
    def __init__(self, cfg):
        ...
        self.fcs = []
        self._output_size = cfg.MODEL.SparseRCNN.HIDDEN_DIM * cfg.MODEL.ROI_BOX_HEAD.POOLER_RESOLUTION * cfg.MODEL.ROI_BOX_HEAD.POOLER_RESOLUTION
        for k in range(num_fc):
            # fc = nn.Linear(self._output_size, fc_dim[k])
            fc = nn.Linear(self._output_size, fc_dim)
            self.add_module("fc{}".format(k + 1), fc)
            self.fcs.append(fc)
            # self._output_size = fc_dim[k]
...
class WSDDNOutputLayers(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.cls = nn.Linear(cfg.MODEL.ROI_BOX_HEAD.FC_DIM, cfg.MODEL.SparseRCNN.NUM_CLASSES)
        self.det = nn.Linear(cfg.MODEL.ROI_BOX_HEAD.FC_DIM, cfg.MODEL.SparseRCNN.NUM_CLASSES)
    ...
    def forward(self, x):
        return F.softmax(self.cls(x), dim=3) * F.softmax(self.det(x), dim=2)


class OICROutputLayers(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.cls_score = nn.Linear(cfg.MODEL.ROI_BOX_HEAD.FC_DIM, cfg.MODEL.SparseRCNN.NUM_CLASSES + 1)
